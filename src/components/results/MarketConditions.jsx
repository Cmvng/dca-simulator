...
import { InfoRow, ToneBadge, Collapsible } from "../ui.jsx";
  ...
  return (
    <div>
      {conditions.components.map((c, i) => (
        <InfoRow key={c.key} label={c.label} last={i === conditions.components.length - 1}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
            {c.detail && <span style={{ color: G.muted, fontWeight: 500 }}>{c.detail}</span>}
            <ToneBadge tone={c.tone}>{c.value}</ToneBadge>
          </span>
        </InfoRow>
      ))}
      <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={{ fontSize: 13, fontWeight: 800, color: G.sub }}>Overall assessment:</span>
        <ToneBadge tone={conditions.overallTone}>{conditions.overall}</ToneBadge>
      </div>
      {conditions.reasons.length > 0 && (
        <ul style={{ margin: "8px 0 0", paddingLeft: 18, fontSize: 13, color: G.muted, lineHeight: 1.6 }}>
