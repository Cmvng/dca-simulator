...
export default function MonteCarloCard({ sim, seed }) {
  ...
  if (!mc.ok) return <div style={body}>{mc.reason}</div>;
  ...
  return (
    <div>
      ...
      {rows.map(([l, v], i) => (
        <SpecRow key={l} label={l} last={i === rows.length - 1}>
      ...
      <div style={{ marginTop: 10 }}>
        <Collapsible title="Methodology & limitations" subtitle={`Deterministic seed ${mc.seed} — same inputs always reproduce this result`}>
          <div style={{ ...body, lineHeight: 1.7 }}>
            ...
            <br /><br />
            <span style={{ fontWeight: 500, color: T.ink }}>Limitations:</span> the model assumes future daily returns resemble the sampled window and are independent day-to-day. Real markets have regime changes, momentum/mean-reversion and fatter tails than any one-year sample. Treat the distribution as a stress-testing tool, not a forecast.
          </div>
        </Collapsible>
      </div>
    </div>
