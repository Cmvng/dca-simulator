...
import { T, SANS, inp, btnOption } from "../styles/theme.js";
import { Collapsible } from "./ui.jsx";

const SLIPPAGE_OPTS = [0, 0.1, 0.25, 0.5, 1];
const HYBRID_OPTS = [0, 10, 20, 30, 50, 70, 90];

const groupLabel = { fontFamily: SANS, fontSize: 13.5, fontWeight: 600, color: T.ink, display: "block", marginBottom: 8 };
const fieldLabel = { fontFamily: SANS, fontSize: 12, fontWeight: 500, color: T.ink2, display: "block", marginBottom: 5 };
const helper = { fontFamily: SANS, fontSize: 12, fontWeight: 400, color: T.ink3, lineHeight: 1.6, marginTop: 6 };
const optChip = active => ({ ...btnOption(active), padding: "7px 13px", fontSize: 12, fontVariantNumeric: "tabular-nums" });
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div role="radiogroup" aria-label="Slippage assumption" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {SLIPPAGE_OPTS.map(s => (
            <button key={s} role="radio" aria-checked={config.slippagePct === s} onClick={() => set("slippagePct", s)}
              style={optChip(config.slippagePct === s)}>
              {s}%
            </button>
          ))}
        </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div role="radiogroup" aria-label="Initial allocation" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {HYBRID_OPTS.map(h => (
            <button key={h} role="radio" aria-checked={config.hybridPct === h} onClick={() => set("hybridPct", h)}
              style={optChip(config.hybridPct === h)}>
              {h}%
            </button>
          ))}
        </div>
      ...
      <div>
        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
          ...
          <span>
            <span style={{ fontFamily: SANS, fontSize: 13.5, fontWeight: 600, color: T.ink, display: "block" }}>Advanced simulation: run 10,000 simulated paths</span>
