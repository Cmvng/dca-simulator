  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 14 }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: G.sub, display: "block", marginBottom: 6 }}>Trading fees (optional)</span>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 130 }}>
            <label htmlFor="fee-pct" style={{ fontSize: 12, color: G.muted, display: "block", marginBottom: 4 }}>Fee per purchase (%)</label>
          ...
          <div style={{ flex: 1, minWidth: 130 }}>
            <label htmlFor="fee-fixed" style={{ fontSize: 12, color: G.muted, display: "block", marginBottom: 4 }}>Fixed fee per purchase ($)</label>
      ...
      {/* Slippage */}
      <div style={{ marginBottom: 14 }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: G.sub, display: "block", marginBottom: 6 }}>
          ...
          {SLIPPAGE_OPTS.map(s => (
            <button key={s} role="radio" aria-checked={config.slippagePct === s} onClick={() => set("slippagePct", s)}
              style={{
                ...
                border: `1.5px solid ${config.slippagePct === s ? G.green : G.border}`,
                background: config.slippagePct === s ? G.greenPale : G.surfaceAlt,
                color: config.slippagePct === s ? G.green : G.muted,
              }}>
              {s}%
            </button>
          ))}
        ...
        <div role="radiogroup" aria-label="Initial allocation" style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {HYBRID_OPTS.map(h => (
            <button key={h} role="radio" aria-checked={config.hybridPct === h} onClick={() => set("hybridPct", h)}
              style={{
                padding: "6px 12px", borderRadius: 10, cursor: "pointer", fontSize: 12, fontWeight: 700,
                border: `1.5px solid ${config.hybridPct === h ? G.green : G.border}`,
                background: config.hybridPct === h ? G.greenPale : G.surfaceAlt,
                color: config.hybridPct === h ? G.green : G.muted,
              }}>
              {h}%
            </button>
          ))}
      ...
      <div>
        <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
          <input type="checkbox" checked={!!config.monteCarlo} onChange={e => set("monteCarlo", e.target.checked)}
            style={{ width: 18, height: 18, accentColor: G.green }} />
          <span>
            <span style={{ fontSize: 13, fontWeight: 700, color: G.sub, display: "block" }}>Advanced simulation: run 10,000 simulated paths</span>
            <span style={{ fontSize: 12, color: G.muted }}>Distribution of outcomes from resampling this coin's historical daily returns (model-based, methodology disclosed in results).</span>
          </span>
        </label>
      </div>
    </Collapsible>
